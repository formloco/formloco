...
const { formSync, importSync, dataSyncToTenant, dataListTenantSync, listsTenant, dataListFormSync, syncDeleteList } = require('../services/syncService')
...
const tenantLists = async(req, res) => {
  try {
    let data =await listsTenant(req.body);
    res.status(201).json(data);
  } catch(e) {
    res.sendStatus(500);
  }
...
module.exports = {
  syncForm, syncImport, syncDataToTenant, syncDataListTenant, tenantLists, syncDataListForm, syncListDelete
};
